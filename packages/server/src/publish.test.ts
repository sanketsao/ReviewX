...
test("snippetTag uses /reviewsx.js when bundled, CDN otherwise", () => {
  assert.match(snippetTag({ project: "p", bundleWidget: true }), /src="\/reviewsx\.js"/);
  assert.match(snippetTag({ project: "p" }), /src="https:\/\/cdn\.jsdelivr\.net\/npm\/reviewsx@1"/);
});

test("basePath prefixes the bundled widget src (GitHub project Pages subpath)", () => {
  assert.match(snippetTag({ project: "p", bundleWidget: true, basePath: "/my-repo/" }), /src="\/my-repo\/reviewsx\.js"/);
...
test("injectSnippet inserts before </body>, once", () => {
  ...
  assert.match(once, /<script[^>]*data-reviewsx[^>]*><\/script>\n<\/body>/);
...
test("staticExport copies files, injects HTML, excludes .protofeedback", async () => {
  ...
  assert.match(await fs.readFile(path.join(out, "index.html"), "utf8"), /data-reviewsx/);
